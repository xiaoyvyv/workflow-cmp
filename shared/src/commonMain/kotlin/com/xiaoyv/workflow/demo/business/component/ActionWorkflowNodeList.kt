package com.xiaoyv.workflow.demo.business.component
...
fun ActionWorkflowNodeList(
    ...
    val nodeMap = remember(workflow) {
        workflow.nodes.associateBy {
            it.id
        }
    }
    val stepMap = remember(steps) {
        steps.associateBy {
            it.nodeId
        }
    }
    ...
    val displayEdges = remember(workflow) {
        buildList {
            ...
            workflow.nodes
                .filter {
                    it.type == ActionNodeType.LOOP_NEXT || it.type == ActionNodeType.LOOP_CONTINUE
                }
                .mapNotNull { node ->
    ...
    val edgeLabels = remember(displayEdges) {
        displayEdges
            .groupBy {
                it.sourceNodeId to it.targetNodeId
            }
            .mapValues { (_, edges) ->
                edges.joinToString(" / ") {
                    it.label
