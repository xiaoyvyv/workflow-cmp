...
import com.xiaoyv.workflow.demo.mvi.UiSideEffect.Loading
import com.xiaoyv.workflow.demo.mvi.UiSideEffect.Toast
import com.xiaoyv.workflow.demo.mvi.UiSideEffect.Wrapped
...
suspend inline fun <T : Any, SIDE_EFFECT : Any> Syntax<UiState<T>, UiSideEffect<SIDE_EFFECT>>.reduceError(
    ...
    state.copy(status = PageStatus.Error(throwable.message.orEmpty(), throwable))
}
...
suspend inline fun <T : Any, SIDE_EFFECT : Any, R> Syntax<UiState<T>, UiSideEffect<SIDE_EFFECT>>.withActionLoading(
    ...
    return block(IntentContext(state.data))
        .onFailure {
            ...
            if (showError) postToast {
                it.message.orEmpty()
            }
        }
        .onSuccess {
            if (enable && autoDismiss) postLoading {
                false
            }
        }
}
