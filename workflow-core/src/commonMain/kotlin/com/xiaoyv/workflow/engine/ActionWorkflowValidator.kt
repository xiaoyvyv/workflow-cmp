...
class ActionWorkflowValidator(private val registry: ActionNodeRegistry) {
    ...
    fun validate(workflow: ActionWorkflow): ActionWorkflowValidation {
        val issues = buildList {
            ...
            if (workflow.nodes.size != nodes.size) {
                add(error(ActionValidationCode.DUPLICATE_NODE_ID, ActionValidationCode.DUPLICATE_NODE_ID_MSG))
            }
            if (workflow.entryNodeId.isBlank() || workflow.entryNodeId !in nodes) {
                add(
                    error(
                        ActionValidationCode.INVALID_ENTRY,
                        ActionValidationCode.INVALID_ENTRY_MSG,
                        nodeId = workflow.entryNodeId.ifBlank { null },
                    )
                )
            }
            if (workflow.globalErrorNodeId != null && workflow.globalErrorNodeId !in nodes) {
                add(
                    error(
                        ActionValidationCode.INVALID_ERROR_NODE,
                        ActionValidationCode.INVALID_ERROR_NODE_MSG,
                        nodeId = workflow.globalErrorNodeId,
                    )
                )
            } else if (
                workflow.globalErrorNodeId != null &&
                findPort(
                ...
                add(
                    error(
                        ActionValidationCode.INVALID_ERROR_NODE,
                        ActionValidationCode.INVALID_ERROR_NODE_IN_PORT_MSG,
                        nodeId = workflow.globalErrorNodeId,
                    )
                )
            }
            workflow.nodes.forEach { node ->
                ...
                if (definition == null) {
                    add(
                        error(
                            ActionValidationCode.UNKNOWN_NODE,
                            "${ActionValidationCode.UNKNOWN_NODE_MSG}：${node.type}",
                            nodeId = node.id,
                        )
                    )
                } else {
                    if (node.nodeVersion > definition.spec.latestVersion) {
                        add(
                            error(
                                ActionValidationCode.UNSUPPORTED_NODE_VERSION,
                                "${ActionValidationCode.UNSUPPORTED_NODE_VERSION_MSG}：${node.type}",
                                nodeId = node.id,
                            )
                        )
                    }
                    definition.spec.requiredConfigKeys
                        ...
                        .forEach { key ->
                            add(
                                error(
                                    ActionValidationCode.MISSING_CONFIG,
                                    "${ActionValidationCode.MISSING_CONFIG_MSG}：$key",
                                    nodeId = node.id,
                                )
                            )
                        }
            ...
            workflow.edges
                .filter { it.kind == ActionPortKind.DATA }
                .forEach { edge ->
                    add(
                        error(
                            ActionValidationCode.UNSUPPORTED_DATA_EDGE,
                            ActionValidationCode.UNSUPPORTED_DATA_EDGE_MSG,
                            edgeId = edge.id,
                        )
                    )
                }
            ...
            if (!declared.containsAll(actual)) {
                add(
                    error(
                        ActionValidationCode.MISSING_CAPABILITY,
                        ActionValidationCode.MISSING_CAPABILITY_MSG,
                    )
                )
            }
        }
        val immutableIssues = issues.toPersistentList()
        return ActionWorkflowValidation(
            issues = immutableIssues,
            isValid = immutableIssues.none { it.level == ActionValidationLevel.ERROR },
        )
    }
    ...
    private fun validateEdges(
        ...
        edgeIds
            ...
            .forEach { id ->
                issues +=
                    error(
                        ActionValidationCode.DUPLICATE_EDGE_ID,
                        ActionValidationCode.DUPLICATE_EDGE_ID_MSG,
                        edgeId = id,
                    )
            }
        edges.forEach { edge ->
            ...
            if (source == null || target == null) {
                issues +=
                    error(
                        ActionValidationCode.MISSING_EDGE_NODE,
                        ActionValidationCode.MISSING_EDGE_NODE_MSG,
                        edgeId = edge.id,
                    )
                return@forEach
            }
            if (edge.source.nodeId == edge.target.nodeId) {
                issues +=
                    error(
                        ActionValidationCode.SELF_EDGE,
                        ActionValidationCode.SELF_EDGE_MSG,
                        edgeId = edge.id,
                    )
            }
            ...
            if (sourcePort == null || targetPort == null)
                issues +=
                    error(
                        ActionValidationCode.INVALID_SOURCE_PORT,
                        ActionValidationCode.INVALID_SOURCE_PORT_MSG,
                        edgeId = edge.id,
                    )
            if (targetPort == null)
                issues +=
                    error(
                        ActionValidationCode.INVALID_TARGET_PORT,
                        ActionValidationCode.INVALID_TARGET_PORT_MSG,
                        edgeId = edge.id,
                    )
            if (
                sourcePort != null &&
                ...
                issues +=
                    error(
                        ActionValidationCode.PORT_KIND_MISMATCH,
                        ActionValidationCode.PORT_KIND_MISMATCH_MSG,
                        edgeId = edge.id,
                    )
            }
        }
        edges
            ...
            .forEach { (target, linkedEdges) ->
                ...
                if (linkedEdges.size > spec.maxConnections) {
                    issues +=
                        error(
                            ActionValidationCode.TOO_MANY_CONNECTIONS,
                            ActionValidationCode.TOO_MANY_CONNECTIONS_MSG,
                            nodeId = node.id,
                        )
                }
            }
        edges
            .groupBy { it.source }
            .forEach { (source, linkedEdges) ->
                val node = nodes[source.nodeId] ?: return@forEach
                val spec = findPort(node.type, source, true) ?: return@forEach
                if (linkedEdges.size > spec.maxConnections) {
                    issues +=
                        error(
                            ActionValidationCode.TOO_MANY_BRANCHES,
                            ActionValidationCode.TOO_MANY_BRANCHES_MSG,
                            nodeId = node.id,
                        )
                }
            }
    ...
    private fun validateControlCycle(
        ...
        val adjacency =
            workflow.edges
                .filter { it.kind == ActionPortKind.CONTROL }
        ...
        if (workflow.nodes.any { visit(it.id) }) {
            issues +=
                error(ActionValidationCode.CONTROL_CYCLE, ActionValidationCode.CONTROL_CYCLE_MSG)
        }
    }

    private fun validateLoops(
        workflow: ActionWorkflow,
        issues: MutableList<ActionValidationIssue>,
    ) {
        val loopTypes =
        ...
        val nodes = workflow.nodes.associateBy { it.id }
        val controlTypes =
            setOf(ActionNodeType.LOOP_NEXT, ActionNodeType.LOOP_CONTINUE, ActionNodeType.LOOP_BREAK)
        workflow.nodes
            .filter { it.type in loopTypes }
            .forEach { node ->
                val ports =
                    workflow.edges
                        .filter { it.kind == ActionPortKind.CONTROL && it.source.nodeId == node.id }
                        .map { it.source.portId }
                        .toSet()
                if (ActionControlPortId.BODY !in ports)
                    issues +=
                        error(
                            ActionValidationCode.LOOP_MISSING_BODY,
                            ActionValidationCode.LOOP_MISSING_BODY_MSG,
                            node.id,
                        )
                if (ActionControlPortId.COMPLETED !in ports)
                    issues +=
                        error(
                            ActionValidationCode.LOOP_MISSING_COMPLETED,
                            ActionValidationCode.LOOP_MISSING_COMPLETED_MSG,
                            node.id,
                        )
                node.config[ActionLoopConfigKey.MAX_ITERATIONS]?.jsonPrimitive?.intOrNull?.let {
                    if (it <= 0)
                        issues +=
                            error(
                                ActionValidationCode.INVALID_LOOP_LIMIT,
                                ActionValidationCode.INVALID_LOOP_LIMIT_MSG,
                                node.id,
                            )
                }
                if (node.type == ActionNodeType.LOOP_REPEAT)
                    node.config[ActionLoopConfigKey.COUNT]?.jsonPrimitive?.intOrNull?.let {
                        if (it < 0)
                            issues +=
                                error(
                                    ActionValidationCode.INVALID_REPEAT_COUNT,
                                    ActionValidationCode.INVALID_REPEAT_COUNT_MSG,
                                    node.id,
                                )
                    }
                val bodyId =
                ...
                if (
                    bodyId != null &&
                    !canReachLoopControl(bodyId, node.id, workflow, nodes, controlTypes)
                ) {
                    issues +=
                        error(
                            ActionValidationCode.LOOP_BODY_NOT_CLOSED,
                            ActionValidationCode.LOOP_BODY_NOT_CLOSED_MSG,
                            node.id,
                        )
                }
                if (
                    bodyId != null && hasLoopEscape(bodyId, node.id, workflow, nodes, controlTypes)
                ) {
                    issues +=
                        error(
                            ActionValidationCode.LOOP_BODY_ESCAPE,
                            ActionValidationCode.LOOP_BODY_ESCAPE_MSG,
                            node.id,
                        )
                }
            }
        workflow.nodes
            .filter { it.type in controlTypes }
            .forEach { node ->
                val loopId = node.config.string(ActionLoopConfigKey.LOOP_ID)
                if (loopId !in nodes || nodes[loopId]?.type !in loopTypes) {
                    issues +=
                        error(
                            ActionValidationCode.INVALID_LOOP_CONTROL_TARGET,
                            ActionValidationCode.INVALID_LOOP_CONTROL_TARGET_MSG,
                            node.id,
                        )
                } else {
                    val bodyId =
                    ...
                    if (bodyId == null || !isReachable(bodyId, node.id, workflow)) {
                        issues +=
                            error(
                                ActionValidationCode.LOOP_CONTROL_OUT_OF_SCOPE,
                                ActionValidationCode.LOOP_CONTROL_OUT_OF_SCOPE_MSG,
                                node.id,
                            )
                    }
                }
     ...
     */
    private fun validateParallelScopes(
        workflow: ActionWorkflow,
        issues: MutableList<ActionValidationIssue>,
    ) {
        val nodes = workflow.nodes.associateBy { it.id }
        ...
        workflow.nodes
            .filter { it.type == ActionNodeType.FLOW_PARALLEL }
            .forEach { parallel ->
                val roots =
                    workflow.edges
                        .filter {
                            it.kind == ActionPortKind.CONTROL &&
                                    it.source.nodeId == parallel.id &&
                                    it.source.portId == ActionControlPortId.BRANCHES
                        }
                ...
                if (roots.isEmpty()) {
                    issues +=
                        error(
                            ActionValidationCode.PARALLEL_MISSING_BRANCH,
                            ActionValidationCode.PARALLEL_MISSING_BRANCH_MSG,
                            parallel.id,
                        )
                    return@forEach
                }
                ...
                if (joins.any { it.isEmpty() }) {
                    issues +=
                        error(
                            ActionValidationCode.PARALLEL_JOIN_MISSING,
                            ActionValidationCode.PARALLEL_JOIN_MISSING_MSG,
                            parallel.id,
                        )
                    return@forEach
                }
                ...
                when {
                    ...
                    commonJoins.size > 1 ->
                        issues +=
                            error(
                                ActionValidationCode.PARALLEL_JOIN_AMBIGUOUS,
                                ActionValidationCode.PARALLEL_JOIN_AMBIGUOUS_MSG,
                                parallel.id,
                            )
                ...
                if (roots.any { hasNestedParallel(workflow, it, nodes) }) {
                    issues +=
                        error(
                            ActionValidationCode.PARALLEL_NESTED,
                            ActionValidationCode.PARALLEL_NESTED_MSG,
                            parallel.id,
                        )
                }
            }
        workflow.nodes
            .filter { it.type == ActionNodeType.FLOW_JOIN && it.id !in pairedJoins }
            .forEach { join ->
                issues +=
                    error(
                        ActionValidationCode.JOIN_NOT_PAIRED,
                        ActionValidationCode.JOIN_NOT_PAIRED_MSG,
                        join.id,
                    )
            }
    }

    private fun reachableJoinNodes(
        workflow: ActionWorkflow,
        startId: String,
        nodes: Map<String, ActionNode>,
    ): Set<String> {
        ...
        while (queue.isNotEmpty()) {
            ...
            if (node.type == ActionNodeType.FLOW_JOIN) {
                ...
                workflow.edges
                    .filter { it.kind == ActionPortKind.CONTROL && it.source.nodeId == nodeId }
    ...
    private fun hasLoopEscape(
        ...
        fun visit(currentId: String): Boolean {
            ...
            if (
                node.type in controlTypes &&
                node.config.string(ActionLoopConfigKey.LOOP_ID) == loopId
            ) {
                return false
            }
            val outgoingEdges =
...
data class ActionWorkflowValidation(
    ...
    /**
     * Materialized for transport consumers (browser and MCP); do not require them to reproduce
     * severity rules.
     */
    val isValid: Boolean,
)
