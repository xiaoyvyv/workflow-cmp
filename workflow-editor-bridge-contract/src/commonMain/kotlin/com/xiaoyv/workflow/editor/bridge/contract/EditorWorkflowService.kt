...
class InMemoryEditorWorkflowRepository : EditorWorkflowRepository {
    ...
    override suspend fun save(
        ...
        val matches =
            if (current == null) expectedRevision == 0L else current.revision == expectedRevision
        if (!matches) return@withLock EditorSaveResult.Conflict(current)
        EditorSaveResult.Saved(
            EditorWorkflowDocument(workflow, expectedRevision + 1).also { documents[id] = it }
        )
    }
}
