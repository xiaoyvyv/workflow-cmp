...
class EditorBridgeServerTest {
    ...
    fun exposesDeviceAndManifestEndpoints() = testApplication {
        ...
        assertTrue(index.bodyAsText().contains("Workflow Editor"))

        val appJs = client.get("/app.js")
        assertEquals(HttpStatusCode.OK, appJs.status)
        assertTrue(appJs.bodyAsText().contains("main.js"))

        val mainJs = client.get("/js/main.js")
        assertEquals(HttpStatusCode.OK, mainJs.status)
        assertTrue(mainJs.bodyAsText().contains("init"))
    }
