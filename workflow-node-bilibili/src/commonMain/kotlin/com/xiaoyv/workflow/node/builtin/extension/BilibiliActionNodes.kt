...
import com.xiaoyv.workflow.node.builtin.extension.BilibiliUtils.completeSearchTypeParameters
import com.xiaoyv.workflow.node.builtin.extension.BilibiliUtils.mixinKeyEncTab
import com.xiaoyv.workflow.node.builtin.extension.BilibiliUtils.removeWbiInvalidCharacters
 ...
 * 哔哩哔哩专用内置节点。
 */
val bilibiliActionNodeDefinitions: List<ActionNodeDefinition> = listOf(bilibiliSignUrlDefinition())

@OptIn(ExperimentalStdlibApi::class)
private fun bilibiliSignUrlDefinition() =
    ActionNodeDefinition(
        spec =
            ActionNodeSpec(
                ...
                requiredConfigKeys =
                    setOf(ActionBilibiliConfigKey.URL, ActionBilibiliConfigKey.OUTPUT_KEY),
                editor = BilibiliNodeEditorCatalog.signUrl,
            ),
        executor = { node, context ->
            val rawUrl =
                ActionTemplateResolver.resolveText(
                    node.config.string(ActionBilibiliConfigKey.URL),
                    context,
                )
            val imgKey =
                node.config[ActionBilibiliConfigKey.IMG_KEY]
                    ?.let { ActionTemplateResolver.resolveText(it.jsonPrimitive.content, context) }
                    .orEmpty()
                    .ifBlank { "7cd084941338484aae1ad9425b84077c" }
            val subKey =
                node.config[ActionBilibiliConfigKey.SUB_KEY]
                    ?.let { ActionTemplateResolver.resolveText(it.jsonPrimitive.content, context) }
                    .orEmpty()
            ...
            val queryString =
                sortedEntries.joinToString("&") { (key, value) ->
                    "$key=${value.removeWbiInvalidCharacters().encodeURLQueryComponent()}"
                }
            val wRid =
                com.appmattus.crypto.Algorithm.MD5.hash(
                    (queryString + mixinKey).encodeToByteArray()
                )
                    .toHexString()
            ...
            node.valueResult(
                node.config.string(ActionBilibiliConfigKey.OUTPUT_KEY),
                JsonPrimitive(signedUrl),
            )
        },
    )

internal object BilibiliUtils {
    internal val mixinKeyEncTab =
        intArrayOf(
            46,
            47,
            18,
    ...
    internal const val BILIBILI_QUERY_VIEW_ID_CHARACTERS =
     ...
     *
     * 调用方只需提供搜索词等业务参数；分页偏移和请求标识由节点根据当前参数生成。 每个默认项都先保留调用方已提供的值；page_size 由调用方完全控制，不在此补齐。
     */
    ...
    internal fun randomQueryViewId(): String =
        buildString(BILIBILI_QUERY_VIEW_ID_LENGTH) {
