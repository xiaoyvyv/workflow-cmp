 ...
 * 日期与时间处理内置节点。
 */
val dateActionNodeDefinitions: List<ActionNodeDefinition> =
    listOf(
        dateNowDefinition(),
    ...
    ActionNodeDefinition(
        spec =
            ActionNodeSpec(
                ...
                editor = CodecNodeEditorCatalog.dateNow,
            ),
        executor = { node, _ ->
            node.valueResult(
...
private fun dateFormatDefinition() =
    ActionNodeDefinition(
        spec =
            ActionNodeSpec(
                ...
                outputPorts = persistentListOf(nextPort),
                requiredConfigKeys =
                    setOf(ActionDateConfigKey.TIMESTAMP, ActionDateConfigKey.OUTPUT_KEY),
                editor = CodecNodeEditorCatalog.dateFormat,
            ),
        executor = { node, context ->
            val timestamp =
                ActionTemplateResolver.resolveElement(
                    node.config[ActionDateConfigKey.TIMESTAMP],
                    context,
                )
                    .jsonPrimitive
                    .content
            ...
            node.valueResult(
                node.config.string(ActionDateConfigKey.OUTPUT_KEY),
                JsonPrimitive(isoString),
            )
        },
    )

private fun dateParseDefinition() =
    ActionNodeDefinition(
        spec =
            ActionNodeSpec(
                ...
                outputPorts = persistentListOf(nextPort),
                requiredConfigKeys =
                    setOf(ActionDateConfigKey.TEXT, ActionDateConfigKey.OUTPUT_KEY),
                editor = CodecNodeEditorCatalog.dateParse,
            ),
        executor = { node, context ->
            val text =
                ActionTemplateResolver.resolveText(
                    node.config.string(ActionDateConfigKey.TEXT),
                    context,
                )
            val epochMs = kotlin.time.Instant.parse(text).toEpochMilliseconds()
            node.valueResult(
                node.config.string(ActionDateConfigKey.OUTPUT_KEY),
                JsonPrimitive(epochMs),
            )
        },
    )

private fun dateAddDefinition() =
    ActionNodeDefinition(
        spec =
            ActionNodeSpec(
                ...
                requiredConfigKeys =
                    setOf(
                        ActionDateConfigKey.TIMESTAMP,
                        ActionDateConfigKey.COUNT,
                        ActionDateConfigKey.OUTPUT_KEY,
                    ),
                editor = CodecNodeEditorCatalog.dateAdd,
            ),
        executor = { node, context ->
            val timestamp =
                ActionTemplateResolver.resolveElement(
            ...
            val count =
                ActionTemplateResolver.resolveElement(
                    node.config[ActionDateConfigKey.COUNT],
                    context,
                )
                    .jsonPrimitive
                    .content
    ...
    ActionNodeDefinition(
        spec =
            ActionNodeSpec(
                ...
                editor = CodecNodeEditorCatalog.dateSubtract,
            ),
        executor = { node, context ->
            val timestamp =
                ActionTemplateResolver.resolveElement(
...
private fun dateDiffDefinition() =
    ActionNodeDefinition(
        spec =
            ActionNodeSpec(
                ...
                requiredConfigKeys =
                    setOf(
                        ActionDateConfigKey.TIMESTAMP_LEFT,
                        ActionDateConfigKey.TIMESTAMP_RIGHT,
                        ActionDateConfigKey.OUTPUT_KEY,
                    ),
                editor = CodecNodeEditorCatalog.dateDiff,
            ),
        executor = { node, context ->
            val left =
                ActionTemplateResolver.resolveElement(
                    node.config[ActionDateConfigKey.TIMESTAMP_LEFT],
                    context,
                )
                    .jsonPrimitive
                    .content
            ...
            val right =
                ActionTemplateResolver.resolveElement(
                    node.config[ActionDateConfigKey.TIMESTAMP_RIGHT],
                    context,
                )
                    .jsonPrimitive
                    .content
...
private fun dateRelativeTimeDefinition() =
    ActionNodeDefinition(
        spec =
            ActionNodeSpec(
                ...
                requiredConfigKeys =
                    setOf(ActionDateConfigKey.TIMESTAMP, ActionDateConfigKey.OUTPUT_KEY),
                editor = CodecNodeEditorCatalog.dateRelativeTime,
            ),
        executor = { node, context ->
            val timestamp =
                ActionTemplateResolver.resolveElement(
...
private fun dateGetComponentDefinition() =
    ActionNodeDefinition(
        spec =
            ActionNodeSpec(
                ...
                requiredConfigKeys =
                    setOf(ActionDateConfigKey.TIMESTAMP, ActionDateConfigKey.OUTPUT_KEY),
                editor = CodecNodeEditorCatalog.dateGetComponent,
            ),
        executor = { node, context ->
            val timestamp =
                ActionTemplateResolver.resolveElement(
...
private fun convertMillisToUnit(millis: Long, unit: String): Double {
    return when (unit.lowercase()) {
        "days",
        "day",
        "d" -> millis.toDouble() / 86_400_000.0

        "hours",
        "hour",
        "h" -> millis.toDouble() / 3_600_000.0

        "minutes",
        "minute",
        "m" -> millis.toDouble() / 60_000.0

        "seconds",
        "second",
        "s" -> millis.toDouble() / 1_000.0
