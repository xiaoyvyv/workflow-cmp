...
private fun jsonExtractDefinition() =
    ActionNodeDefinition(
        spec =
            ActionNodeSpec(
                ...
                requiredConfigKeys =
                    setOf(
                        ActionJsonConfigKey.SOURCE,
                        ActionJsonConfigKey.PATH,
                        ActionJsonConfigKey.OUTPUT_KEY,
                    ),
                editor = CodecNodeEditorCatalog.jsonExtract,
            ),
        executor = { node, context ->
            val source =
                ActionTemplateResolver.resolveElement(
                    node.config[ActionJsonConfigKey.SOURCE],
                    context,
                )
...
private fun jsonParseDefinition() =
    ActionNodeDefinition(
        spec =
            ActionNodeSpec(
                ...
                outputPorts = persistentListOf(nextPort),
                requiredConfigKeys =
                    setOf(ActionJsonConfigKey.TEXT, ActionJsonConfigKey.OUTPUT_KEY),
                editor = CodecNodeEditorCatalog.jsonParse,
            ),
        executor = { node, context ->
            node.valueResult(
                ...
                defaultJson.parseToJsonElement(
                    ActionTemplateResolver.resolveText(
                        node.config.string(ActionJsonConfigKey.TEXT),
                        context,
                    )
                ),
            )
...
private fun jsonStringifyDefinition() =
    ActionNodeDefinition(
        spec =
            ActionNodeSpec(
                ...
                outputPorts = persistentListOf(nextPort),
                requiredConfigKeys =
                    setOf(ActionJsonConfigKey.VALUE, ActionJsonConfigKey.OUTPUT_KEY),
                editor = CodecNodeEditorCatalog.jsonStringify,
            ),
        executor = { node, context ->
            node.valueResult(
                node.config.string(ActionJsonConfigKey.OUTPUT_KEY),
                JsonPrimitive(
                    ActionTemplateResolver.resolveElement(
                        node.config[ActionJsonConfigKey.VALUE],
                        context,
                    )
                        .toString()
                ),
...
private fun jsonValidateDefinition() =
    ActionNodeDefinition(
        spec =
            ActionNodeSpec(
                ...
                inputPorts = persistentListOf(inPort),
                outputPorts =
                    persistentListOf(nextPort, com.xiaoyv.workflow.node.builtin.failurePort),
                requiredConfigKeys =
                    setOf(ActionJsonConfigKey.TEXT, ActionJsonConfigKey.OUTPUT_KEY),
                editor = CodecNodeEditorCatalog.jsonValidate,
            ),
        executor = { node, context ->
            val text =
                ActionTemplateResolver.resolveText(
            ...
            ActionNodeExecutionResult(
                outputPortId =
                    if (isValid) ActionControlPortId.NEXT else ActionControlPortId.FAILURE,
                output =
                    kotlinx.serialization.json.buildJsonObject { put(key, JsonPrimitive(isValid)) },
            )
        },
    )

private fun jsonSchemaValidateDefinition() =
    ActionNodeDefinition(
        spec =
            ActionNodeSpec(
                ...
                requiredConfigKeys =
                    setOf(ActionJsonConfigKey.VALUE, ActionJsonConfigKey.OUTPUT_KEY),
                editor = CodecNodeEditorCatalog.jsonSchemaValidate,
            ),
        executor = { node, context ->
            val element =
                ActionTemplateResolver.resolveElement(
            ...
            val schema =
                node.config[ActionJsonConfigKey.SCHEMA]?.let {
