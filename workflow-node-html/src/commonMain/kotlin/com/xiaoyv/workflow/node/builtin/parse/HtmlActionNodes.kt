...
import com.xiaoyv.workflow.node.core.int
...
/**
 * 解析单段 HTML 文本为 [Element]。 若为完整 HTML 文档则返回 Document 根元素；若为片段则返回 body 下的首个子元素或 body 自身。
 */
...
/**
 * 解析输入源为 [Elements] 集合。 支持传入 HTML 字符串数组或单个 HTML 字符串。
 */
...
/**
 * 从节点配置中解析多态目标 [HtmlTarget]。 支持单个 HTML 字符串或 HTML 字符串数组。
 */
 ...
 */
val htmlActionNodeDefinitions: List<ActionNodeDefinition> =
    listOf(
        htmlParseDefinition(),
 ...
 * 解析并格式化/清洗 HTML 源码。
 */
private fun htmlParseDefinition() =
    ActionNodeDefinition(
        spec =
            ActionNodeSpec(
                ...
                editor = HtmlNodeEditorCatalog.htmlParse,
            ),
 ...
 */
private fun htmlSelectDefinition() =
    ActionNodeDefinition(
        spec =
            ActionNodeSpec(
                ...
                outputPorts = persistentListOf(nextPort),
                requiredConfigKeys =
                    setOf(ActionHtmlConfigKey.SELECTOR, ActionHtmlConfigKey.OUTPUT_KEY),
                editor = HtmlNodeEditorCatalog.htmlSelect,
            ),
        executor = { node, context ->
            val target = resolveHtmlTarget(node, context)
            val selector =
                ActionTemplateResolver.resolveText(
                    node.config.string(ActionHtmlConfigKey.SELECTOR),
                    context,
                )
            val elements =
                when (target) {
 ...
 */
private fun htmlSelectFirstDefinition() =
    ActionNodeDefinition(
        spec =
            ActionNodeSpec(
                ...
                requiredConfigKeys =
                    setOf(ActionHtmlConfigKey.SELECTOR, ActionHtmlConfigKey.OUTPUT_KEY),
                editor = HtmlNodeEditorCatalog.htmlSelectFirst,
            ),
        executor = { node, context ->
            val target = resolveHtmlTarget(node, context)
            val selector =
 ...
 * 提取标签属性值。
 */
private fun htmlAttrDefinition() =
    ActionNodeDefinition(
        spec =
            ActionNodeSpec(
                ...
                outputPorts = persistentListOf(nextPort),
                requiredConfigKeys =
                    setOf(ActionHtmlConfigKey.ATTRIBUTE, ActionHtmlConfigKey.OUTPUT_KEY),
                editor = HtmlNodeEditorCatalog.htmlAttr,
            ),
        executor = { node, context ->
            val target = resolveHtmlTarget(node, context)
            val attribute =
                ActionTemplateResolver.resolveText(
                    node.config.string(ActionHtmlConfigKey.ATTRIBUTE),
                    context,
                )
    ...
    ActionNodeDefinition(
        spec =
            ActionNodeSpec(
                ...
                outputPorts = persistentListOf(nextPort),
                requiredConfigKeys = setOf(ActionHtmlConfigKey.OUTPUT_KEY),
                editor = HtmlNodeEditorCatalog.htmlTag,
            ),
        executor = { node, context ->
            val tagName =
                when (val target = resolveHtmlTarget(node, context)) {
 ...
 * 提取纯文本内容。
 */
private fun htmlTextDefinition() =
    ActionNodeDefinition(
        spec =
            ActionNodeSpec(
                ...
                outputPorts = persistentListOf(nextPort),
                requiredConfigKeys = setOf(ActionHtmlConfigKey.OUTPUT_KEY),
                editor = HtmlNodeEditorCatalog.htmlText,
            ),
        executor = { node, context ->
            val textValue =
                when (val target = resolveHtmlTarget(node, context)) {
 ...
 */
private fun htmlDataDefinition() =
    ActionNodeDefinition(
        spec =
            ActionNodeSpec(
                ...
                outputPorts = persistentListOf(nextPort),
                requiredConfigKeys = setOf(ActionHtmlConfigKey.OUTPUT_KEY),
                editor = HtmlNodeEditorCatalog.htmlData,
            ),
        executor = { node, context ->
            val dataValue =
                when (val target = resolveHtmlTarget(node, context)) {
 ...
 * 提取输入控件值（value）。
 */
private fun htmlValueDefinition() =
    ActionNodeDefinition(
        spec =
            ActionNodeSpec(
                ...
                outputPorts = persistentListOf(nextPort),
                requiredConfigKeys = setOf(ActionHtmlConfigKey.OUTPUT_KEY),
                editor = HtmlNodeEditorCatalog.htmlValue,
            ),
        executor = { node, context ->
            val valueStr =
                when (val target = resolveHtmlTarget(node, context)) {
    ...
    ActionNodeDefinition(
        spec =
            ActionNodeSpec(
                ...
                outputPorts = persistentListOf(nextPort),
                requiredConfigKeys = setOf(ActionHtmlConfigKey.OUTPUT_KEY),
                editor = HtmlNodeEditorCatalog.htmlId,
            ),
        executor = { node, context ->
            val idValue =
                when (val target = resolveHtmlTarget(node, context)) {
 ...
 * 提取内部 HTML (innerHTML)。
 */
private fun htmlHtmlDefinition() =
    ActionNodeDefinition(
        spec =
            ActionNodeSpec(
                ...
                outputPorts = persistentListOf(nextPort),
                requiredConfigKeys = setOf(ActionHtmlConfigKey.OUTPUT_KEY),
                editor = HtmlNodeEditorCatalog.htmlHtml,
            ),
        executor = { node, context ->
            val htmlValue =
                when (val target = resolveHtmlTarget(node, context)) {
 ...
 */
private fun htmlOuterHtmlDefinition() =
    ActionNodeDefinition(
        spec =
            ActionNodeSpec(
                ...
                outputPorts = persistentListOf(nextPort),
                requiredConfigKeys = setOf(ActionHtmlConfigKey.OUTPUT_KEY),
                editor = HtmlNodeEditorCatalog.htmlOuterHtml,
            ),
        executor = { node, context ->
            val outerHtmlValue =
                when (val target = resolveHtmlTarget(node, context)) {
 ...
 * 检查是否包含指定 CSS 类名。
 */
private fun htmlHasClassDefinition() =
    ActionNodeDefinition(
        spec =
            ActionNodeSpec(
                ...
                outputPorts = persistentListOf(nextPort),
                requiredConfigKeys =
                    setOf(ActionHtmlConfigKey.CLASS_NAME, ActionHtmlConfigKey.OUTPUT_KEY),
                editor = HtmlNodeEditorCatalog.htmlHasClass,
            ),
        executor = { node, context ->
            val target = resolveHtmlTarget(node, context)
            val className =
                ActionTemplateResolver.resolveText(
                    node.config.string(ActionHtmlConfigKey.CLASS_NAME),
                    context,
                )
    ...
    ActionNodeDefinition(
        spec =
            ActionNodeSpec(
                ...
                outputPorts = persistentListOf(nextPort),
                requiredConfigKeys =
                    setOf(ActionHtmlConfigKey.OPERATION, ActionHtmlConfigKey.OUTPUT_KEY),
                editor = HtmlNodeEditorCatalog.htmlMap,
            ),
        executor = { node, context ->
            val elements = resolveHtmlElements(node, context)
            val op =
                ActionTemplateResolver.resolveText(
                    node.config.string(ActionHtmlConfigKey.OPERATION),
                    context,
                )
                    .lowercase()
            val attrName =
                node.config[ActionHtmlConfigKey.ATTRIBUTE]?.let {
                    ActionTemplateResolver.resolveText(
                        node.config.string(ActionHtmlConfigKey.ATTRIBUTE),
                        context,
                    )
            ...
            val mapped = elements.map { el ->
                when (op) {
                    ...
                    "attr" -> {
                        require(!attrName.isNullOrBlank()) {
                            "html.map 执行 attr 操作时必须指定 attribute 参数"
                        }
 ...
 */
private fun htmlFirstDefinition() =
    ActionNodeDefinition(
        spec =
            ActionNodeSpec(
                ...
                outputPorts = persistentListOf(nextPort),
                requiredConfigKeys = setOf(ActionHtmlConfigKey.OUTPUT_KEY),
                editor = HtmlNodeEditorCatalog.htmlFirst,
            ),
        executor = { node, context ->
            val elements = resolveHtmlElements(node, context)
 ...
 */
private fun htmlLastDefinition() =
    ActionNodeDefinition(
        spec =
            ActionNodeSpec(
                ...
                outputPorts = persistentListOf(nextPort),
                requiredConfigKeys = setOf(ActionHtmlConfigKey.OUTPUT_KEY),
                editor = HtmlNodeEditorCatalog.htmlLast,
            ),
        executor = { node, context ->
            val elements = resolveHtmlElements(node, context)
    ...
    ActionNodeDefinition(
        spec =
            ActionNodeSpec(
                ...
                outputPorts = persistentListOf(nextPort),
                requiredConfigKeys =
                    setOf(ActionHtmlConfigKey.INDEX, ActionHtmlConfigKey.OUTPUT_KEY),
                editor = HtmlNodeEditorCatalog.htmlGet,
            ),
        executor = { node, context ->
            val elements = resolveHtmlElements(node, context)
            val index = node.config.int(ActionHtmlConfigKey.INDEX)
            val element =
                elements.getOrNull(index)?.let { JsonPrimitive(it.outerHtml()) } ?: JsonNull
 ...
 * 统计 HTML 集合中包含的元素数量。
 */
private fun htmlSizeDefinition() =
    ActionNodeDefinition(
        spec =
            ActionNodeSpec(
                ...
                outputPorts = persistentListOf(nextPort),
                requiredConfigKeys = setOf(ActionHtmlConfigKey.OUTPUT_KEY),
                editor = HtmlNodeEditorCatalog.htmlSize,
            ),
        executor = { node, context ->
            val elements = resolveHtmlElements(node, context)
            node.valueResult(
                node.config.string(ActionHtmlConfigKey.OUTPUT_KEY),
                JsonPrimitive(elements.size),
            )
        },
    )
 ...
 */
private fun htmlRemoveDefinition() =
    ActionNodeDefinition(
        spec =
            ActionNodeSpec(
                ...
                requiredConfigKeys =
                    setOf(ActionHtmlConfigKey.SELECTOR, ActionHtmlConfigKey.OUTPUT_KEY),
                editor = HtmlNodeEditorCatalog.htmlRemove,
            ),
        executor = { node, context ->
            val target = resolveHtmlTarget(node, context)
            val selector =
 ...
 * 提取元素的直接父级 outerHtml。
 */
private fun htmlParentDefinition() =
    ActionNodeDefinition(
        spec =
            ActionNodeSpec(
                ...
                outputPorts = persistentListOf(nextPort),
                requiredConfigKeys = setOf(ActionHtmlConfigKey.OUTPUT_KEY),
                editor = HtmlNodeEditorCatalog.htmlParent,
            ),
        executor = { node, context ->
            val parent =
                when (val target = resolveHtmlTarget(node, context)) {
 ...
 */
private fun htmlChildrenDefinition() =
    ActionNodeDefinition(
        spec =
            ActionNodeSpec(
                ...
                outputPorts = persistentListOf(nextPort),
                requiredConfigKeys = setOf(ActionHtmlConfigKey.OUTPUT_KEY),
                editor = HtmlNodeEditorCatalog.htmlChildren,
            ),
        executor = { node, context ->
            val children =
                when (val target = resolveHtmlTarget(node, context)) {
 ...
 */
private fun htmlTableToJsonDefinition() =
    ActionNodeDefinition(
        spec =
            ActionNodeSpec(
                ...
                outputPorts = persistentListOf(nextPort),
                requiredConfigKeys = setOf(ActionHtmlConfigKey.OUTPUT_KEY),
                editor = HtmlNodeEditorCatalog.htmlTableToJson,
            ),
        executor = { node, context ->
            val rawHtml =
                node.config[ActionHtmlConfigKey.HTML]
            ...
            val selector =
                node.config[ActionHtmlConfigKey.SELECTOR]?.let {
                    ActionTemplateResolver.resolveText(
                        node.config.string(ActionHtmlConfigKey.SELECTOR),
                        context,
                    )
