package com.xiaoyv.workflow.ui
...
class WorkflowSideEffectHostState {
    ...
    suspend fun dispatch(effect: ActionSideEffect): ActionSideEffectResult {
        ...
        return suspendCancellableCoroutine { continuation ->
            ...
            continuation.invokeOnCancellation {
                confirmQueue = confirmQueue.filterNot {
                    it.id == entryId
                }
                inputQueue = inputQueue.filterNot {
                    it.id == entryId
                }
                selectQueue = selectQueue.filterNot {
                    it.id == entryId
                }
                syncCookieQueue = syncCookieQueue.filterNot {
                    it.id == entryId
                }
                oneShotQueue = oneShotQueue.filterNot {
                    it.id == entryId
                }
            }
        }
    ...
    private fun applyProgressEffect(effect: ActionProgressDialogEffect) {
        val taskId = effect.taskId.ifBlank {
            "default"
        }
        when (effect.action) {
            ...
            ActionProgressDialogAction.UPDATE -> {
                ...
                if (existingIndex >= 0) {
                    ...
                    val updatedEffect = old.effect.copy(
                        title = effect.title.ifBlank {
                            old.effect.title
                        },
                        message = effect.message.ifBlank {
                            old.effect.message
                        },
                        mode = effect.mode.ifBlank {
                            old.effect.mode
                        },
                ...
                val data = WorkflowSideEffectData(taskId, normalizedEffect) {}
                val existingIndex = progressTasks.indexOfFirst {
                    it.id == taskId
                }
                progressTasks = if (existingIndex >= 0) {
                    progressTasks.toMutableList().apply {
                        set(existingIndex, data)
                    }
                } else {
                    progressTasks + data
                }
